"""
handlers/browse.py — 💤 выход в меню, уведомления о лайках
"""
import re
...
async def _notify_liked(bot: Bot, liker_id: int, target_id: int):
    try:
        ...
        if not liker_profile: return
        await bot.send_message(
            target_id,
            "💌 <b>Ваша анкета кому-то понравилась!</b>\n\nПосмотрите — может это ваша половинка? 🌙",
            parse_mode="HTML"
        )
        await send_profile_card(
...
async def _notify_mutual(bot: Bot, user_a: int, user_b: int):
    pa = await db.get_profile(user_a)
    pb = await db.get_profile(user_b)
    ...
    if ub.get("username"):
        try:
            await bot.send_message(
                user_a,
                f"🎉 <b>Взаимная симпатия!</b>\n\n<b>{pb['name']}</b> тоже поставил(а) лайк!\n\nОбщайтесь через махрама 🤝",
                parse_mode="HTML", reply_markup=write_keyboard(ub["username"])
            )
    ...
    if ua.get("username"):
        try:
            await bot.send_message(
                user_b,
                f"🎉 <b>Взаимная симпатия!</b>\n\n<b>{pa['name']}</b> тоже поставил(а) лайк!\n\nОбщайтесь через махрама 🤝",
                parse_mode="HTML", reply_markup=write_keyboard(ua["username"])
            )
        except Exception: pass
...
# !! ВАЖНО: BTN_SLEEP должен быть ПЕРВЫМ хендлером в browsing
...
async def do_like(message: Message, state: FSMContext, bot: Bot):
    ...
    if not ok: await message.answer(err); return
...
async def do_dislike(message: Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    target_id = data.get("current_profile_id")
    if target_id: await db.add_like(message.from_user.id, target_id, -1)
    user = await db.get_user(message.from_user.id)
    await _show_next(bot, message.chat.id, user, state)


@router.message(BrowseStates.browsing, F.text == BTN_LIKE_MSG)
async def like_msg_start(message: Message, state: FSMContext):
    user = await db.get_user(message.from_user.id)
    if not user["is_premium"]:
        await message.answer(f"⭐ Только Премиум. @{ADMIN_USERNAME}"); return
    await message.answer("Тип сообщения:", reply_markup=like_message_type_keyboard())
...
async def like_msg_type(call: CallbackQuery, state: FSMContext):
    await call.answer()
    ...
    prompts = {"text": "✍️ Текст:", "audio": "🎤 Голосовое:", "video_note": "🎥 Видеокружок:"}
    await call.message.answer(prompts[action])


@router.message(BrowseStates.like_message)
async def like_msg_receive(message: Message, state: FSMContext, bot: Bot):
    data = await state.get_data()
    ...
    if msg_type == "text":
        if not message.text: await message.answer("Текст:"); return
        if re.search(r"@\w+|https?://|t\.me/", message.text, re.I):
            await message.answer("⛔ Без ссылок."); return
        msg_text = message.text.strip()
    elif msg_type == "audio":
        if not message.voice: await message.answer("Голосовое:"); return
        msg_file = message.voice.file_id
    elif msg_type == "video_note":
        if not message.video_note: await message.answer("Видеокружок:"); return
        msg_file = message.video_note.file_id
    user = await db.get_user(message.from_user.id)
    ok, err = await _check_like_limit(user)
    if not ok: await message.answer(err); await state.set_state(BrowseStates.browsing); return
    await db.add_like(message.from_user.id, target_id, 1,
