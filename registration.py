"""
handlers/registration.py — регистрация, анкета, редактирование
Добавлено: обработка /start ref_XXXX
"""
...
from utils import (
    check_subscription, contains_links_or_mentions,
...
async def cmd_start(message: Message, state: FSMContext, bot: Bot):
    ...
    # Реферальный параметр: /start ref_123456789
    ...
    # Привязка реферала (только для новых пользователей)
    if ref_code and not user:
        from handlers.referral import handle_referral_start
        await handle_referral_start(user_id, ref_code, bot)
    ...
    if profile:
        ...
        await message.answer(
            "С возвращением! 🌙",
            reply_markup=main_menu(u["gender"], has_profile=True)
        )
        return
...
async def get_name(message: Message, state: FSMContext):
    ...
    await message.answer("Ваш возраст (от 18 до 80):")
...
async def get_city(message: Message, state: FSMContext):
    ...
    if len(city) < 2 or contains_links_or_mentions(city):
...
async def get_about(message: Message, state: FSMContext):
    ...
    if contains_links_or_mentions(about):
        await message.answer("⛔ Текст не должен содержать ссылки или @упоминания:")
        return
    if len(about) < ABOUT_MIN_CHARS:
        await message.answer(
            f"❗ Слишком коротко ({len(about)} симв.). Минимум {ABOUT_MIN_CHARS}:"
        )
        return
    await state.update_data(about=about)
    await message.answer(
        f"📸 Загрузите фото (до {MAX_PHOTOS} штук).\n"
        "Можно отправить все сразу альбомом.\n"
        "⚠️ Без фото анкета не публикуется.",
        reply_markup=cancel_keyboard()
    )
...
async def get_photos(message: Message, state: FSMContext):
    ...
    if len(photos) >= MAX_PHOTOS:
        ...
        await message.answer(
            f"✅ Фото {len(photos)}/{MAX_PHOTOS}. Ещё или /done:"
        )


@router.message(RegStates.waiting_photos, F.text.in_({"/done", "done"}))
...
async def _finish_photos(message: Message, state: FSMContext):
    ...
    preview = {
        ...
        "city": data["city"], "about": data["about"],
        "gender": data["gender"], "username": message.from_user.username,
        "is_premium": False,
    }
...
async def confirm_profile(call: CallbackQuery, state: FSMContext):
    await call.answer()
    data = await state.get_data()
    ...
    await call.message.answer(
        ...
        reply_markup=main_menu(data["gender"], has_profile=True)
    )
...
async def _save_field(tg_id, field, value):
    p = await db.get_profile(tg_id)
    kwargs = {
        "name": p["name"], "age": p["age"],
        "city": p["city"], "about": p["about"]
    }
    kwargs[field] = value
    await db.save_profile(tg_id, **kwargs)
...
async def edit_name_save(message: Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear(); await message.answer("Отменено.", reply_markup=remove_kb()); return
    name = message.text.strip()
    if len(name) < 2 or contains_links_or_mentions(name):
        await message.answer("Некорректное имя:"); return
    await _save_field(message.from_user.id, "name", name)
    await state.clear()
    u = await db.get_user(message.from_user.id)
    await message.answer("✅ Имя обновлено!", reply_markup=main_menu(u["gender"], True))
...
async def edit_age_save(message: Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear(); await message.answer("Отменено.", reply_markup=remove_kb()); return
    try:
        age = int(message.text.strip()); assert 18 <= age <= 80
    except (ValueError, AssertionError):
        await message.answer("Введите корректный возраст (18–80):"); return
    await _save_field(message.from_user.id, "age", age)
    await state.clear()
    u = await db.get_user(message.from_user.id)
    await message.answer("✅ Возраст обновлён!", reply_markup=main_menu(u["gender"], True))
...
async def edit_city_save(message: Message, state: FSMContext):
    ...
    if contains_links_or_mentions(city):
        await message.answer("⛔ Некорректный город:"); return
    await _save_field(message.from_user.id, "city", city)
    await state.clear()
    u = await db.get_user(message.from_user.id)
    await message.answer("✅ Город обновлён!", reply_markup=main_menu(u["gender"], True))
...
async def edit_about_save(message: Message, state: FSMContext):
    ...
    if contains_links_or_mentions(about):
        await message.answer("⛔ Текст содержит ссылки:"); return
    if len(about) < ABOUT_MIN_CHARS:
        await message.answer(f"❗ Минимум {ABOUT_MIN_CHARS} симв. Сейчас: {len(about)}:"); return
    await _save_field(message.from_user.id, "about", about)
    await state.clear()
    u = await db.get_user(message.from_user.id)
    await message.answer("✅ Описание обновлено!", reply_markup=main_menu(u["gender"], True))
...
async def edit_photos_start(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await call.message.answer(
        f"Отправьте новые фото (до {MAX_PHOTOS}). По завершении — /done",
        reply_markup=cancel_keyboard()
    )
...
async def edit_photos_receive(message: Message, state: FSMContext):
    ...
    if len(photos) >= MAX_PHOTOS:
        ...
        await state.clear()
        u = await db.get_user(message.from_user.id)
        await message.answer("✅ Фото обновлены!", reply_markup=main_menu(u["gender"], True))
    else:
...
async def edit_photos_done(message: Message, state: FSMContext):
    ...
    photos = data.get("photos", [])
    if not photos:
        await message.answer("⚠️ Нужно хотя бы одно фото."); return
    await db.save_photos(message.from_user.id, photos)
    await state.clear()
    u = await db.get_user(message.from_user.id)
    await message.answer("✅ Фото обновлены!", reply_markup=main_menu(u["gender"], True))
...
async def edit_cancel(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Отменено.", reply_markup=main_menu(u["gender"], True))
