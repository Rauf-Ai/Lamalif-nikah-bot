"""
handlers/misc.py — раздел Профиль (инлайн меню), поддержка, симпатии
"""
...
async def prof_view(call: CallbackQuery, bot: Bot):
    ...
    await send_profile_card(
        bot, call.message.chat.id, profile, photos,
        show_username=True,
        extra_text=f"Статус: {status}"
    )
...
async def prof_toggle_hide(call: CallbackQuery):
    ...
    msg = (
        "🔴 Анкета скрыта. Вас не видят в ленте."
        if new_hidden else
        "🟢 Анкета снова активна! Вас видят другие. 🌙"
    )
...
async def prof_support(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await _show_support(call.from_user.id, call.message, state, edit=False)


async def _show_support(user_id: int, message, state: FSMContext, edit: bool = False):
...
async def support_send_reply(message: Message, state: FSMContext, bot: Bot):
    ...
    if not ticket:
        await message.answer("Тикет не найден."); await state.clear(); return
    await db.reply_ticket(ticket_id, message.text)
    try:
        await bot.send_message(
            ticket["user_id"],
            f"💬 <b>Ответ поддержки #{ticket_id}:</b>\n\n{message.text}\n\n"
            "Ещё вопросы — напишите через раздел Профиль → Поддержка.",
            parse_mode="HTML"
        )
...
async def premium_info(message: Message):
    ...
    if user and user["is_premium"]:
        ...
        await message.answer(
            f"✅ Премиум активен до <b>{until}</b>",
            parse_mode="HTML",
...
async def help_cmd(message: Message):
    await message.answer(
        "🌙 <b>ЛямАлиф Никях</b>\n\n"
        "👀 <b>Смотреть анкеты</b> — ставьте лайки\n"
        "👤 <b>Профиль</b> — анкета, симпатии, рефералы, поддержка\n"
